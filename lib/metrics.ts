...
/** True when the manual `Total Cost` / `Time to Hire (week)` entry disagrees
 * with the computed value by more than a small tolerance — surfaced for review
...
export function timeToFillDays(r: HireRecord): number | null {
  ...
  const days = (r.resumptionDate.getTime() - r.requisitionStartDate.getTime()) / DAY_MS;
  return days;
}
...
/**
 * Accepted / (Accepted + Declined + Withdrawn) — Pending offers are excluded
 * from the denominator because their outcome isn't known yet. See
 * docs/02-metrics-dictionary.md for the alternative (include Pending) if that
 * turns out to be the preferred definition.
 */
...
function groupAvgBy(records: HireRecord[], keyFn: (r: HireRecord) => string): GroupedAvg[] {
  ...
  return [...map.entries()]
    .map(([key, values]) => ({ key, avg: avg(values) ?? 0, count: values.length }))
...
export function costPerHireTrend(records: HireRecord[]): MonthPoint[] {
  ...
  return [...map.entries()]
    .map(([month, values]) => ({ month, value: avg(values) ?? 0 }))
...
function velocityRankingBy(records: HireRecord[], keyFn: (r: HireRecord) => string): VelocityRanking[] {
  ...
  return [...map.entries()]
    .map(([key, values]) => ({ key, avgDays: avg(values) ?? 0, count: values.length }))
...
export function hiringSeasonality(
  records: HireRecord[],
  ...
  return [...map.entries()]
    .map(([month, rows]) => {
      ...
      return {
        ...
        avgTimeToFillDays: avg(
          acceptedRows.map(timeToFillDays).filter((d): d is number => d != null)
        ),
